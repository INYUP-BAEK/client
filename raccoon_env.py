...
class SyncSimRaccoonEnv:
    ...
    def __init__(
        ...
        if not 0.0 <= smooth_alpha <= 1.0:
            raise ValueError(f"smooth_alpha must be in [0.0, 1.0], got {smooth_alpha}")
        if max_action_abs is not None and max_action_abs <= 0.0:
            raise ValueError(f"max_action_abs must be positive when provided, got {max_action_abs}")
        ...
        self.max_action_abs = float(max_action_abs) if max_action_abs is not None else None
        self._previous_smoothed_delta = None
        ...
        if log_csv_path is not None:
            self.enable_csv_logging(log_csv_path)

        self.reset_episode(0.15, 0.15, 0.0)

    # ---------- optional V3 execution logging ----------
    ...
    @staticmethod
    def _norm(values) -> Optional[float]:
        if values is None:
            return None
        arr = np.asarray(values, dtype=np.float64).reshape(-1)
        return float(np.linalg.norm(arr))

    def enable_csv_logging(self, log_csv_path: str) -> None:
        if self._log_csv_file is not None:
            self._log_csv_file.close()
            self._log_csv_file = None
            self._log_csv_writer = None

        path = Path(log_csv_path)
        path.parent.mkdir(parents=True, exist_ok=True)
        self._log_csv_file = open(path, "w", newline="", encoding="utf-8")
        fieldnames = [
            "step_index",
        ...
        self._log_csv_writer = csv.DictWriter(self._log_csv_file, fieldnames=fieldnames)
        self._log_csv_writer.writeheader()
        self._log_csv_file.flush()

    def _write_execution_log_row(self, info: Dict[str, object]) -> None:
        if self._log_csv_writer is None or self._log_csv_file is None:
            return

        row = {
            "step_index": info.get("step_index"),
            "timestamp_perf_counter": info.get("timestamp_perf_counter"),
            "execution_time_s": info.get("execution_time_s"),
            "raw_action": self._json_dumps(info.get("raw_action")),
            "applied_delta": self._json_dumps(info.get("applied_delta")),
            "executed_delta": self._json_dumps(info.get("executed_delta")),
            "target_position": self._json_dumps(info.get("target_position")),
            "actual_move": self._json_dumps(info.get("actual_move")),
            "retries": info.get("retries"),
            "raw_action_norm": info.get("raw_action_norm"),
            "applied_delta_norm": info.get("applied_delta_norm"),
            "executed_action_norm": info.get("executed_action_norm"),
            "action_delta_norm": info.get("action_delta_norm"),
            "done": info.get("done"),
            "success": info.get("success"),
            "use_action_smoothing": info.get("use_action_smoothing"),
            "smooth_alpha": info.get("smooth_alpha"),
            "max_action_abs": info.get("max_action_abs"),
            "clipped_delta": self._json_dumps(info.get("clipped_delta")),
            "smoothed_delta": self._json_dumps(info.get("smoothed_delta")),
            "raw_delta_norm": info.get("raw_delta_norm"),
            "clipped_delta_norm": info.get("clipped_delta_norm"),
            "smoothed_delta_norm": info.get("smoothed_delta_norm"),
            "smoothing_delta_norm": info.get("smoothing_delta_norm"),
            "step_delta_norm": info.get("step_delta_norm"),
        }
        self._log_csv_writer.writerow(row)
        self._log_csv_file.flush()

    def _postprocess_movement_delta(self, raw_delta: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float, float]:
        if self.max_action_abs is None:
            clipped_delta = raw_delta.astype(np.float64, copy=True)
        else:
            clipped_delta = np.clip(raw_delta, -self.max_action_abs, self.max_action_abs).astype(np.float64)

        previous_smoothed_delta = self._previous_smoothed_delta
        if self.use_action_smoothing and previous_smoothed_delta is not None:
            smoothed_delta = (
                self.smooth_alpha * clipped_delta
                + (1.0 - self.smooth_alpha) * previous_smoothed_delta
            )
        else:
            smoothed_delta = clipped_delta.astype(np.float64, copy=True)

        if previous_smoothed_delta is None:
            step_delta_norm = 0.0
        else:
            step_delta_norm = self._norm(smoothed_delta - previous_smoothed_delta)

        smoothing_delta_norm = self._norm(smoothed_delta - clipped_delta)
        self._previous_smoothed_delta = smoothed_delta.astype(np.float64, copy=True)
        return clipped_delta, smoothed_delta, smoothing_delta_norm, step_delta_norm
    ...
    def execute_delta_action7(
        ...
        raw_delta_np = np.asarray([raw_dx, raw_dy, raw_dz], dtype=np.float64)
        clipped_delta_np, smoothed_delta_np, smoothing_delta_norm, step_delta_norm = self._postprocess_movement_delta(
            raw_delta_np
        )
        dx, dy, dz = [float(v) for v in smoothed_delta_np]
        ...
        applied_delta = [dx, dy, dz]
        executed_delta = [final_dx, final_dy, final_dz]
        target_position = [tx, ty, tz]
        execution_time_s = time.perf_counter() - start_time
        raw_delta = [raw_dx, raw_dy, raw_dz]
        clipped_delta = clipped_delta_np.tolist()
        smoothed_delta = smoothed_delta_np.tolist()

        info = {
            "step_index": step_index,
            "success": True,
            "done": False,
            "timestamp_perf_counter": start_time,
            "execution_time_s": execution_time_s,
            "ee_pose_before": [ee_x, ee_y, ee_z],
            "raw_action": raw_action,
            "raw_delta_xyz": [raw_dx, raw_dy, raw_dz],
            "applied_delta_xyz": applied_delta,
            "final_delta_xyz": executed_delta,
            ...
            "retries": len(tried_results) - 1,
            "raw_action_norm": self._norm(raw_action),
            "raw_delta_norm": self._norm(raw_delta),
            "clipped_delta_norm": self._norm(clipped_delta),
            "smoothed_delta_norm": self._norm(smoothed_delta),
            "applied_delta_norm": self._norm(applied_delta),
            "executed_action_norm": self._norm(executed_delta),
            "smoothing_delta_norm": smoothing_delta_norm,
            "step_delta_norm": step_delta_norm,
            "action_delta_norm": self._norm(np.asarray(raw_delta, dtype=np.float64) - np.asarray(executed_delta, dtype=np.float64)),
        }
        self._write_execution_log_row(info)
        return info
